...
public class ViewAddRemoveRoles {
    ...
    */
    ...
    // ADDED BY KENNY NGUYEN: centered column baseline for absolute positions
    private static final double CONTENT_W = 720; 
    private static final double COL_X = (width - CONTENT_W) / 2.0; 
    private static final double GAP_X = 20;
    ...
    */
    ...
    public ViewAddRemoveRoles() {
        ...
        theAddRemoveRolesScene.getStylesheets().add(
        	    ViewAddRemoveRoles.class.getResource("/styles/app.css").toExternalForm()
        	);
        
        // ADDED BY KENNY NGUYEN: force dark background on Scene
        theAddRemoveRolesScene.setFill(javafx.scene.paint.Color.web("#1e1e1e"));
        theRootPane.setPrefSize(width, height);
        theRootPane.getStyleClass().add(".page-root");
        
        // Populate the window with the title and other common widgets and set their static state
        
        // GUI Area 1
        label_PageTitle.setText("Add/Remove Roles Page"); // EDITED BY KENNY NGUYEN: fixed title text
        setupLabelUI(label_PageTitle, "Arial", 28, width, Pos.CENTER, 0, 5);

        label_UserDetails.setText("User: " + theUser.getUserName());
        setupLabelUI(label_UserDetails, "Arial", 20, width, Pos.BASELINE_LEFT, COL_X + GAP_X, 55); // EDITED BY KENNY NGUYEN
        
        setupButtonUI(button_UpdateThisUser, "Dialog", 18, 170, Pos.CENTER, COL_X + CONTENT_W - 170 - GAP_X, 45); // EDITED BY KENNY NGUYEN
        button_UpdateThisUser.setOnAction((event) -> 
            {guiUserUpdate.ViewUserUpdate.displayUserUpdate(theStage, theUser); });
        
        /*****ADDED BY KENNY NGUYEN: Apply CSS classes to labels and buttons
        label_PageTitle.getStyleClass().add("title");
     	...
     	label_CurrentRoles.getStyleClass().add("form-label");
     	label_SelectRoleToBeAdded.getStyleClass().add("form-label");
     	label_SelectRoleToBeRemoved.getStyleClass().add("form-label");
     	
     	*******/
        
        // GUI Area 2a
        setupLabelUI(label_SelectUser, "Arial", 20, 300, Pos.BASELINE_LEFT, COL_X + GAP_X, 130); // EDITED BY KENNY NGUYEN
        
        setupComboBoxUI(combobox_SelectUser, "Dialog", 16, 280, COL_X + GAP_X + 260, 125); // EDITED BY KENNY NGUYEN
        ...
        // GUI Area 2b
        setupLabelUI(label_CurrentRoles, "Arial", 16, 480, Pos.BASELINE_LEFT, COL_X + GAP_X, 170);    // EDITED BY KENNY NGUYEN
        setupLabelUI(label_SelectRoleToBeAdded, "Arial", 20, 300, Pos.BASELINE_LEFT, COL_X + GAP_X, 210); // EDITED BY KENNY NGUYEN
        setupComboBoxUI(combobox_SelectRoleToAdd, "Dialog", 16, 180, COL_X + GAP_X + 260, 205); // EDITED BY KENNY NGUYEN
        setupButtonUI(button_AddRole, "Dialog", 16, 160, Pos.CENTER, COL_X + GAP_X + 260 + 200, 205); // EDITED BY KENNY NGUYEN

        ViewAddRemoveRoles.button_AddRole.setOnAction((event) -> 
            {ControllerAddRemoveRoles.performAddRole(); });

        setupLabelUI(label_SelectRoleToBeRemoved, "Arial", 20, 300, Pos.BASELINE_LEFT, COL_X + GAP_X, 280); // EDITED BY KENNY NGUYEN
        setupComboBoxUI(combobox_SelectRoleToRemove, "Dialog", 16, 180, COL_X + GAP_X + 260, 275);    // EDITED BY KENNY NGUYEN
        setupButtonUI(button_RemoveRole, "Dialog", 16, 160, Pos.CENTER, COL_X + GAP_X + 260 + 200, 275); // EDITED BY KENNY NGUYEN

        ViewAddRemoveRoles.button_RemoveRole.setOnAction((event) -> 
            {ControllerAddRemoveRoles.performRemoveRole(); });
        
        // GUI Area 3        
        setupButtonUI(button_Return, "Dialog", 18, 210, Pos.CENTER, COL_X + GAP_X, 540); // EDITED BY KENNY NGUYEN
        button_Return.setOnAction((event) -> {ControllerAddRemoveRoles.performReturn(); });

        setupButtonUI(button_Logout, "Dialog", 18, 210, Pos.CENTER, COL_X + GAP_X + 280, 540); // EDITED BY KENNY NGUYEN
        button_Logout.setOnAction((event) -> {ControllerAddRemoveRoles.performLogout(); });
    
        setupButtonUI(button_Quit, "Dialog", 18, 210, Pos.CENTER, COL_X + GAP_X + 560, 540); // EDITED BY KENNY NGUYEN
