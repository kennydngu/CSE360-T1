...
public class ControllerNewAccount {
	...
	*/
	 ...
	 * 
	 */	
	public static boolean valid_username_check (String message) {
	...
	protected static void doCreateUser() {
		...
		String message_username = userNameRecognizer.UserNameRecognizer.checkForValidUserName(username);
		System.out.println(message_username);
		String password = ViewNewAccount.text_Password1.getText();
		
		String message_password = passwordPopUpWindow.Model.evaluatePassword(password);
		...
		if (!message_password.isEmpty()) {
			...
			ViewNewAccount.alertUsernamePasswordError.setContentText("Invalid Password: " + 
		message_password.replace("\n*** ERROR ***", " ").trim());
		...
		if (!message_username.isEmpty()) {
			ViewNewAccount.alertUsernamePasswordError.setTitle("UserName is not valid");
			ViewNewAccount.alertUsernamePasswordError.setContentText("Invalid Username: " + 
		message_username.replace("\n*** ERROR ***", " ").trim());
