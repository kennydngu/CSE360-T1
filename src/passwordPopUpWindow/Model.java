...
public class Model {
	...
	public static boolean foundShortEnough = false;		// ADDED BY KENNY NGUYEN: New condition password must meet. password must be shorter than 32 characters
	...
	private static void updateFlags() {
		...
		// ADDED BY KENNY NGUYEN: Added label in the gui that updates the red condition not met label
		if (foundShortEnough) {
			View.label_ShortEnough.setText("32 or less characters - Satisfied");
			View.label_ShortEnough.setTextFill(Color.GREEN);
			
			// UPDATES short enough so that if it is too long it will then show red
		if (!foundShortEnough) {
			View.label_ShortEnough.setText("32 or more characters found too long! - Unsatisfied");
	...
	public static String evaluatePassword(String input) {
		...
		// ADDED BY KENNY NGUYEN: If the password length is greater than 32 it will not be accepted.
		foundShortEnough = (input.length() <= 32);
		if(input.length() > 32) {
		...
		foundShortEnough = false; 			// ADDED by KENNY NGUYEN: Reset the boolean flag
		...
		while (running) {
			...
			if (currentCharNdx <= 32) {
				System.out.println("At most 32 characters or less found");
		...
		if (!foundShortEnough)				// ADDED BY KENNY NGUYEN: If the password is too long it also adds too the errMessage
			errMessage += "Too long; ";
